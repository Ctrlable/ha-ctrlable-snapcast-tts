...
async def announce(client_id: str, tts_url: str, source_host: str) -> AnnounceResult:
    ...
    async with _lock(client_id):
        # Auto-detect home group on first announcement
        if not cs.home_group_id:
            snap_clients = await snap.list_clients()
            match = next((c for c in snap_clients if c.id == client_id), None)
            if match:
                cs.home_group_id = match.current_group_id
                cs.home_group_autodetected = True
                save_state()
                _LOGGER.info("Auto-detected home group %r for %r", cs.home_group_id, client_id)

        # Move to announce group only when it differs from home (music clients)
        needs_restore = False
        if cs.announce_group_id and cs.announce_group_id != cs.home_group_id:
        ...
        try:
            ...
            if needs_restore and cs.home_group_id:
                with contextlib.suppress(Exception):
